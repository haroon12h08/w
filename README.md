# Bank 4.0 Core Financial System (research phases 1–9: ledger, banking domain, payments, lending, point-in-time history, counterfactual evaluation, credit information, outcome research, research sensitivity)
...
- **Flyway**: Versioned, reproducible SQL migrations (`V1__reference_data.sql` through `V14__research_sensitivity.sql`).
...
### Research sensitivity (`/api/v1/research/sensitivity`)
See [docs/research-sensitivity.md](docs/research-sensitivity.md). How descriptive results change with the horizon, the knowledge cutoff and the population, and why: every change is attributed to a configuration parameter. Knowledge cutoffs are always explicit timestamps; every result is a reproducible artefact.
- `POST /horizons`, `POST /knowledge-cutoffs`, `POST /comparisons`, `POST /boundaries`, `POST /population`
- `GET /reports/{id}`, `GET /reports/{id}/reproduction`
